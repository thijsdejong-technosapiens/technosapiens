...
class Scan extends Event {
	...
	public function save_settings( Request $request ): Response {
		...
		} elseif ( $this->model->is_enabled_scheduled_scanning() && false === $data['scheduled_scanning'] ) {
			$report         = new Malware_Report();
			$report_change  = true;
	...
	public function get_issues( Request $request ): Response {
		...
		if ( ! $v->validate() ) {
			return new Response(
				false,
				array(
					'message' => esc_html__( 'Wrong scan issue data.', 'wpdef' ),
				)
			);
		}
	...
	public function data_frontend(): array {
		...
		if ( $settings->is_enabled_scheduled_scanning() && isset( $settings->frequency ) ) {
			$report_text = sprintf(
	...
	public function config_strings( array $config ): array {
		...
		$is_pro = property_exists( $this, 'is_pro' ) ? $this->is_pro : wd_di()->get( WPMUDEV::class )->is_pro();
